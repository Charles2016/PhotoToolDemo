...
- (void)setSourceType:(MLSourceType)sourceType {
    ...
    self.videoView.hidden = (sourceType == MLSourcePhoto);
}
...
- (void)setAnimationRightTick:(BOOL)animationRightTick{
    ...
    if (animationRightTick) {
        [self.tickImageView setImage:[UIImage imageNamed:MLSelectPhotoSrcName(@"photo_choose_icon")] forState:UIControlStateNormal];
    }else{
        [self.tickImageView setImage:[UIImage imageNamed:MLSelectPhotoSrcName(@"photo_notchoose_icon")] forState:UIControlStateNormal];
    }
    ...
    if (self.sourceType == MLSourceVideo) {
